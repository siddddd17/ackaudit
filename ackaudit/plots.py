...
def fig_proxy_vs_true(results: list[dict], path: Path) -> None:
    ...
    fig, axes = plt.subplots(
        1, len(labels), figsize=(4.2 * len(labels), 3.8), squeeze=False
    )
        ...
        for solver in SOLVER_ORDER:
            rs = sorted(
                [
                    r
                    for r in results
                    if r["ok"] and r["label"] == label and r["solver"] == solver
                ],
            ...
            if not rs:
                continue
            ax.plot(
                [r["budget"] for r in rs],
                [r["proxy_peak_memory"] for r in rs],
                linestyle="--",
                alpha=0.45,
                marker="o",
                markersize=3,
                label=f"{solver} (proxy)",
            )
            ax.plot(
                [r["budget"] for r in rs],
                [r["true_peak_memory"] for r in rs],
                linestyle="-",
                marker="s",
                markersize=3,
                label=f"{solver} (true)",
    ...
    fig.suptitle(
        "Objective the solver optimises (dashed) vs. simulated backward peak (solid)",
        fontsize=11,
    )
    fig.tight_layout()
...
def fig_solver_cost(results: list[dict], path: Path) -> None:
    ...
    for solver in SOLVER_ORDER:
        vals = [
            r["solver_seconds"] * 1000
            for r in results
            if r["ok"] and r["solver"] == solver
        ]
        if vals:
            data.append(vals)
            names.append(solver)
