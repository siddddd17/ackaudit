...
predecessors, and so on. The closure of that walk is the set of activations that
must exist simultaneously to produce n. The knapsack weighs each item by its own
tensor size alone, so the size of this closure is invisible to the objective the
solver optimises.
...
class Cell:
    ...
    # summed node memory of each closure, in the solver's own units
...
def report(cells: list[Cell]) -> str:
    ...
    for c in sorted(ok, key=lambda c: c.budget):
        ...
        out.append(
            ...
            f"{s['weight_median']:>9.4f} {s['weight_max']:>8.4f}"
        )
    out.append("")
    out.append(
        "size columns are node counts; w columns sum the solver's own memory units."
    )
    return "\n".join(out) + "\n"
